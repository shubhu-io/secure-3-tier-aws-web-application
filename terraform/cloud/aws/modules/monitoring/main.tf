...
locals {
  ...
  # ALB ARN: ...:loadbalancer/app/<name>/<id>  -> drop the "app" segment
  alb_suffix = join("/", slice(split("/", var.alb_arn), 2, length(split("/", var.alb_arn))))
  # TG  ARN: ...:targetgroup/<name>/<id>        -> keep both segments
  tg_suffix = join("/", slice(split("/", var.target_group_arn), 1, length(split("/", var.target_group_arn))))
}
...
resource "aws_cloudwatch_metric_alarm" "alb_target_4xx" {
  alarm_name          = "${local.name_prefix}-alb-target-4xx"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = "2"
  ...
  alarm_description   = "Target 4xx responses above ${var.alb_4xx_threshold} in 10 minutes"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.alerts.arn]
...
resource "aws_cloudwatch_metric_alarm" "alb_target_response_time" {
  ...
  alarm_description   = "Average target response time above ${var.alb_response_time_threshold}s for 10 minutes"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.alerts.arn]
...
resource "aws_cloudwatch_metric_alarm" "alb_request_count_low" {
  alarm_name          = "${local.name_prefix}-alb-request-count-low"
  ...
  alarm_description   = "ALB request count below ${var.alb_request_count_threshold} per 5 minutes - the app may be unreachable"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.alerts.arn]
...
resource "aws_cloudwatch_metric_alarm" "rds_connections_high" {
  ...
  alarm_description   = "RDS connections above ${var.rds_connections_threshold} for 10 minutes"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.alerts.arn]
  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      },
      {
        ...
        height = 6
        properties = {
          markdown = "### ${local.name_prefix}\n\n**Metric → Alarm → SNS → Email**\n\nAlarms fire when:\n- ASG CPU > ${var.cpu_high_threshold}%\n- ALB target 5xx > ${var.alb_5xx_threshold}\n- ALB target 4xx > ${var.alb_4xx_threshold}\n- ALB target response time > ${var.alb_response_time_threshold}s\n- ALB request count < ${var.alb_request_count_threshold}\n- A target is unhealthy\n- RDS CPU > ${var.rds_cpu_threshold}%\n- RDS connections > ${var.rds_connections_threshold}\n- RDS free storage < 20%"
        }
      }
    ]
  })
